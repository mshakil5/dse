...
class DivisionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $chkname = Division::where('name',$request->name)->first();
        if($chkname){
    ...
    public function edit($id)
    {
        ...
        $info = Division::where($where)->get()->first();
    ...
    public function update(Request $request)
    {
        ...
        $duplicatename = Division::where('name',$request->name)->where('id','!=', $request->codeid)->first();
        if($duplicatename){
            $message ="<div class='alert alert-warning'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><b>This name already added.</b></div>";
            return response()->json(['status'=> 303,'message'=>$message]);
            exit();
        ...
        $data = Division::find($request->codeid);
        $data->name = $request->name;
